...
def run_deck(cfg: dict) -> None:
    from agents.visualizer.agent.image_fetcher import fetch_images_for_slides
    from agents.visualizer.agent.intake import collect_deck_intake
    from agents.visualizer.agent.outline import approval_checkpoint, generate_outline
    from agents.visualizer.agent.renderer import render_deck
    from agents.visualizer.agent.slide_writer import generate_slide_content
    from agents.visualizer.agent.viz_classifier import classify_slides

    model = cfg["model"]
    slides_dir = AGENT_DIR / cfg["output"]["slides_dir"].replace("agent/", "")
    template_path = AGENT_DIR / cfg["template_path"].replace("agent/", "")
...
def run_image(cfg: dict) -> None:
    from agents.visualizer.agent.image_fetcher import fetch_single_image
    from agents.visualizer.agent.intake import collect_image_intake
    from agents.visualizer.agent.viz_classifier import propose_image_concepts

    model = cfg["model"]
    images_dir = AGENT_DIR / cfg["output"]["images_dir"].replace("agent/", "")
    img_w = cfg["image"]["width"]
    img_h = cfg["image"]["height"]
