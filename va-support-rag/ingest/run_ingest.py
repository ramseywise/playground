...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Ingest JSONL corpus into three-store layout"
    )
    parser.add_argument(
        "--source",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            os.getenv("VECTORDB_PATH") or "../data/stores/vectordb/clara.duckdb"
        ),
    ...
    n_docs = con.execute(
        "SELECT COUNT(DISTINCT doc_id) FROM ingest_documents"
    ).fetchone()[0]
    con.close()
