...
_WORDS_PER_TOKEN: float = 0.75  # conservative estimate for multilingual text
...
def _merge_with_overlap(pieces: list[str], config: ChunkerConfig) -> list[str]:
    ...
    for piece in pieces:
        ...
        if _approx_tokens(" ".join(tentative)) <= config.max_tokens:
            current_words = tentative
        else:
            if current_words:
                text_out = " ".join(current_words)
                if _approx_tokens(text_out) >= config.min_tokens:
