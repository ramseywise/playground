...
async def run_variant_experiment(
    ...
    *,
    ...
    the real Bedrock KB API when ``retrieval_strategy == "bedrock"``.
    Logs traces + scores to LangFuse when available.

    Args:
        variant_name: Key from VARIANTS (librarian, raptor, bedrock, bedrock-live).
        golden_samples: Golden queries with expected doc URLs.
        corpus: Chunks to populate InMemoryRetriever (ignored for bedrock-live).
    ...
    # Dispatch to real Bedrock KB API when configured
...
async def run_all_experiments(
    ...
    *,
    ...
    for name, cfg in variant_configs.items():
        # Skip bedrock-live when credentials are absent
