"""Canonical chunk data models shared across storage and librarian.

These live in ``core`` so that ``storage`` backends can import them without
creating an upward dependency on the ``librarian`` domain package.

Dependency rule:
    storage → core ← librarian     (correct)
    storage → librarian             (inversion — avoid)
"""
