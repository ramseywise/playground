...
from agents.visualizer.image_fetcher import fetch_images_for_slides, fetch_single_image
from agents.visualizer.models import VizPrompt
...
def test_fetch_images_skips_skip_image(tmp_path: Path) -> None:
    ...
    with patch("agents.visualizer.image_fetcher.get_provider", return_value=mock_provider):
...
def test_fetch_images_parallel(tmp_path: Path) -> None:
    ...
    with patch("agents.visualizer.image_fetcher.get_provider", return_value=mock_provider):
        with patch("agents.visualizer.image_fetcher.settings") as mock_settings:
