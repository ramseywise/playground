...
def test_cache_hit_miss(monkeypatch) -> None:
    ...
    monkeypatch.setattr("agents.librarian.pipeline.retrieval.cache.time.monotonic", lambda: 1.0)
...
def test_cache_expiry(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "agents.librarian.pipeline.retrieval.cache.time.monotonic", lambda: next(times)
    )
    cache.put("q", "inmemory", 5, [_result()])
