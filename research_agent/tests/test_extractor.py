...
from research_agent.extractor import extract_pages, extract_toc, get_page_count
...
def test_get_page_count_parses_pdfinfo_output() -> None:
    ...
    with patch("research_agent.extractor.subprocess.run", return_value=mock_result):
...
def test_extract_toc_calls_extract_pages_with_first_10() -> None:
    with patch("research_agent.extractor.extract_pages", return_value="toc text") as mock_ep:
...
def test_extract_toc_custom_pages() -> None:
    with patch("research_agent.extractor.extract_pages", return_value="toc") as mock_ep:
