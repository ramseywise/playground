...
from agents.research_agent.src.research_agent.chunker import Chunk, plan_chunks
from agents.research_agent.src.research_agent.extractor import get_page_count, extract_pages
from agents.research_agent.src.research_agent.models import Note, NoteMetadata, resolve_topic
from agents.research_agent.src.research_agent.prompts import (
    SYSTEM_PROMPT,
    build_merge_prompt,
    build_note_prompt,
)
from agents.research_agent.src.paths import DROPBOX_READINGS, OBSIDIAN_TOPICS
...
def _relative_source(pdf_path: Path) -> str:
    try:
        return str(pdf_path.relative_to(DROPBOX_READINGS))
    except ValueError:
