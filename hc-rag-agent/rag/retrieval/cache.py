...
class RetrievalCache:
    """Thread-safe TTL LRU cache keyed on (query, strategy, top_k)."""
    ...
    def put(
        self, query: str, strategy: str, top_k: int, results: list[RetrievalResult]
    ) -> None:
        key = self._key(query, strategy, top_k)
    ...
    def _key(query: str, strategy: str, top_k: int) -> tuple[str, str, int]:
        return query.strip().lower(), strategy.strip().lower(), top_k
