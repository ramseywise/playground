...
def _make_retriever_agent(
    *,
    ...
    agent.run = AsyncMock(
        ...
        or {"graded_chunks": [], "retrieved_chunks": [], "query_variants": []}
    )
    return agent
...
def _make_reranker_agent(
    *,
    ...
    agent.run = AsyncMock(
        return_value=run_return or {"reranked_chunks": [], "confidence_score": 0.0}
    )
    return agent
...
async def test_search_knowledge_base_returns_results() -> None:
    graded = [
        GradedChunk(
            chunk=_make_chunk("c1", "First passage about auth."),
            score=0.95,
            relevant=True,
        ),
        GradedChunk(
            chunk=_make_chunk("c2", "Second passage about billing."),
            score=0.72,
            relevant=True,
        ),
    ]
    retriever_agent = _make_retriever_agent(
        run_return={
            ...
            "retrieved_chunks": [],
            "query_variants": ["what is authentication?"],
        },
    )
...
async def test_search_knowledge_base_respects_num_results() -> None:
    graded = [
        GradedChunk(
            chunk=_make_chunk(f"c{i}", f"Passage {i}"),
            score=0.9 - i * 0.1,
            relevant=True,
        )
        for i in range(5)
...
async def test_condense_query_rewrites_with_agent() -> None:
    ...
    condenser = _make_condenser_agent(
        condense_return={
            "standalone_query": "How does SAML compare to OAuth for authentication?"
        },
    )
